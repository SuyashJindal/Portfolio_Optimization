...
class PortfolioManager:
    ...
    def get_data(tickers: List[str], start: str, end: str, benchmark: Optional[str] = None) -> Tuple[pd.DataFrame, Optional[pd.Series]]:
        """Fetch historical price data with caching and robust error handling."""
        ...
        try:
            if len(tickers) == 1:
                # Single ticker case
                data = yf.download(tickers[0], start=start, end=end, progress=False, auto_adjust=True)
                if data.empty:
                    raise ValueError(f"No data returned for {tickers[0]}")
                
                # For single ticker, use Close price
                if 'Close' in data.columns:
                    price_data = pd.DataFrame({tickers[0]: data['Close']})
                else:
                    price_data = pd.DataFrame({tickers[0]: data.iloc[:, 0]})
            else:
                # Multiple tickers
                ...
                for ticker in tickers:
                    try:
                        if len(tickers) > 1:
                            # Multi-ticker: columns are (Ticker, PriceType)
                ...
                if price_data.empty:
                    raise ValueError("Could not parse any price data from response")
        
        except Exception as e:
            raise ValueError(f"Data fetch failed: {str(e)}. Check tickers and date range.")
        ...
        if benchmark:
            try:
                bench_data = yf.download(benchmark, start=start, end=end, progress=False, auto_adjust=True)
                if not bench_data.empty and 'Close' in bench_data.columns:
                    bench_series = bench_data['Close']
                elif not bench_data.empty:
                    bench_series = bench_data.iloc[:, 0]
            except Exception:
                st.warning(f"Could not fetch benchmark {benchmark}")
        ...
        return price_data, bench_series
    ...
    def get_metrics(weights: np.ndarray, returns: pd.DataFrame, rf: float = 0.0, 
        ...
        metrics = {
            ...
            "Omega Ratio": round(omega if omega != np.inf else 999.99, 4),
...
class Optimizer:
    ...
    def __init__(self, returns: pd.DataFrame, constraints: Dict, rf: float = 0.0, 
        ...
        self.mean_rets = returns.mean() * 252  # Annualized
    ...
    def optimize(self, method: str, confidence: float = 0.95) -> Dict[str, float]:
        ...
        try:
            ...
            if not res.success:
                st.warning(f"⚠️ Optimization warning: {res.message}")
            ...
            equal_w = 1.0 / self.num_assets
            return dict(zip(self.tickers, [equal_w] * self.num_assets))
    ...
    def calculate_frontier(self, num_points: int = 25) -> Tuple[List[float], List[float]]:
        ...
        try:
            ...
            st.warning(f"Frontier calculation failed: {str(e)}")
            return [], []
...
with col_d1:
    start_date = st.date_input("Start", default_start, help="Data start date")
with col_d2:
    end_date = st.date_input("End", default_end, help="Data end date")
